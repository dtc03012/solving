...
void recur(int now,int bit,int upbit,int tot,int st)
{
	ma=max(ma,tot);
	if(n==now) return;
	for(int e=st;e<m;e++)
	{
		if(e<m-1)
		{
			...
			if(dp[now][nextbit]<tot+a[tmp[now][e]-'A'][tmp[now][e+1]-'A'])
			{
				...
				recur(now,nextbit,upbit,tot+a[tmp[now][e]-'A'][tmp[now][e+1]-'A'],st+2);
			}
		}
		if(now>0) 
		{
			if((upbit&(1<<e))==0) 
			{
				...
				if(dp[now][nextbit]<tot+a[tmp[now][e]-'A'][tmp[now-1][e]-'A'])
				{
					...
					recur(now,nextbit,(upbit|(1<<e)),tot+a[tmp[now][e]-'A'][tmp[now-1][e]-'A'],st+1);
				}
			}
		}
		recur(now,bit,upbit,tot,e+1);
	}
	recur(now+1,0,bit,tot,0);
} 
int main(void)
{
